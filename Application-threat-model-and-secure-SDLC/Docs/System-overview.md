...
| Item | System Component Identity / Details | Portfolio Context Label | User for Each Item |
| :--- | :--- | :--- | :--- |
| **Business Service** | Customer storefront browsing, authentication workflows, checkout routing, and order metadata processing. | Simulated Business Context | Anonymous Visitor, Authenticated Customer |
| **Critical Application** | **AST-001** (Customer Web Application / API Gateway front-end edge layer). | Reused from Enterprise Inventory | Anonymous Visitor, Authenticated Customer, Support Agent, Finance User |
| **Critical Database** | **AST-002** (Relational storage repository retaining historical profiles). | Reused from Enterprise Inventory | Application Administrator, CI/CD Service Identity (Indirectly via Process) |
| **Cloud Environment** | **AST-003** (AWS cloud infrastructure environment deployed in region `eu-central-1`). | Simulated Operational Boundary | Application Administrator, Security Analyst |
| **Application Server** | **AST-004** (Ubuntu Linux instance hosting application core processing structures). | Reused from Enterprise Inventory | Application Administrator, Developer (Staging/Deploy environments) |
| **Workforce Identity** | **AST-006** (Microsoft Entra ID identity provider managing corporate staff tracking). | Simulated Use / Asset Reused | Support Agent, Finance User, Application Administrator, Security Analyst |
| **Source Control** | **AST-009** (GitHub source code repositories and integration hooks). | Reused from Enterprise Inventory | Developer, CI/CD Service Identity |
| **Payment Service** | **AST-010** (Stripe software-as-a-service processing integration boundary). | Reused from Enterprise Inventory | Authenticated Customer (Via Redirect), Finance User (Refund calls) |
| **Audit Log Source** | **AST-013** (AWS CloudTrail native configuration tracking and telemetry loops). | Reused from Enterprise Inventory | Security Analyst, Application Administrator |
| **SIEM Destination** | **AST-014** (Hosted Splunk instance managing corporate security operations ingestion). | Reused from Enterprise Inventory | Security Analyst |
| **Restricted Data (Identity)** | **DAT-001** (Customer Identity Profile Data: credentials, addresses, tokens). | Reused from Enterprise Inventory | Authenticated Customer, Support Agent (Masked read) |
| **Restricted Data (Orders)** | **DAT-002** (Order/Payment Metadata: transaction totals, invoice tokens). | Reused from Enterprise Inventory | Authenticated Customer, Support Agent, Finance User |
| **Configuration Data** | **DAT-004** (Confidential application source code and cloud deployment parameters). | Reused from Enterprise Inventory | Developer, CI/CD Service Identity |
| **Security Logs Data** | **DAT-006** (Confidential infrastructure activity records and application security trails). | Reused from Enterprise Inventory | Security Analyst |
...
*   **Recovery Point Objective (RPO):** **1 Hour** maximum acceptable data loss threshold applied explicitly to the primary transaction application database (**AST-002** / **DS-01**) through automated, point-in-time transaction logs shipping [Simulated Baseline].
